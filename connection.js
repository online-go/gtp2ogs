...
class Connection {
    ...
    checkGameSettings(notification) { /* {{{ */
        ...
        function minmaxHandicapFamilyReject(arg) {
            // first, we define rankedUnranked and minMax depending on arg
            let rankedUnranked = "";
            if (!arg.includes("ranked")) {
                rankedUnranked = "";
            }
            else if (arg.includes("ranked") && !arg.includes("unranked")) {
                rankedUnranked = "for ranked games";
            }
            else if (arg.includes("unranked")) {
            ...
            if (notification.handicap === 0 && minMax === "Min" && arg > 0) {
                conn_log(`handicap games only ${rankedUnranked}`);
                return { reject: true, msg: `this bot does not play even games ${rankedUnranked}, please manually select the number of handicap stones in -custom handicap- : minimum is ${arg} handicap stones or more` };
            }
            else if (notification.handicap > 0 && minMax === "Max" && arg === 0) {
            ...
            // then finally, the actual reject :
                ...
                conn_log(`${minMax} handicap ${rankedUnranked} is ${arg}`);
                return { reject: true, msg: `${minMax} handicap ${rankedUnranked} is ${arg}, please ${increaseDecrease} the number of handicap stones` };
            }
        }
